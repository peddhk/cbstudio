...
(function () {
  ...
  // API base configurável via <meta name="cb-api-base" content="https://...">.
  // Se vazio, usa o mesmo domínio (caminhos relativos).
  ...
  function renderSuccess(key) {
    ...
    result.querySelector('[data-action="download"]').addEventListener('click', () => {
      window.location.href = api(`/api/download/${encodeURIComponent(key)}`);
    });
    result.querySelector('[data-action="view"]').addEventListener('click', () => {
      window.open(api(`/api/view/${encodeURIComponent(key)}`), '_blank', 'noopener');
    });
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch(api(`/api/status/${encodeURIComponent(raw)}`));
